...
void sender() {
  ...
  if (turn == 's') //sender turn
  {
    ...
    {
      printf("SENDER: Sent packet with SEQ NO : %d", p.seq);
      ...
      printf("\n%s", (errorframe == 0 ? "Error While sending Packet" : ""));
      turn = 'r';
...
void receiver() {
  ...
  if (turn == 'r') {
    if (errorframe != 0) {
      ...
      {
        printf("RECEIVER: Received packet with SEQ NO : %d", p.seq);
        ...
        printf("RECEIVER: Duplicated packet with seq %d", frexp - 1);
